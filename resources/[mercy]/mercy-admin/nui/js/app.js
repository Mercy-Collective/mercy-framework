...
$(document).on("click", ".menu-current-target", function (e) {
  $(this)
    .parent()
    .find(".ui-styles-input")
    .each(function (Elem, Obj) {
      if ($(this).find("input").data("PlayerId")) {
        if (MC.AdminMenu.CurrentTarget != null) {
          if (
            $(".admin-menu-item")
              .find(".admin-menu-items-option-input")
              .first()
              .find(".ui-input-label")
              .text() == "Player"
          ) {
...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  MC.AdminMenu.Command.SelectedCat = $(
    ".menu-page-commands-header-categories"
  ).find(".active");
  ...
  window.addEventListener("message", function (event) {
    ...
    switch (Action) {
      ...
      case "SetItemEnabled":
        ...
        Data.State
          ? $(`#admin-option-${Data.Name}`).addClass("enabled")
          : $(`#admin-option-${Data.Name}`).removeClass("enabled");
        break;
      case "Copy":
